...
The preCICE tutorials included in the distribution can not only be used as reference examples on how to couple different adapters, but also to verify functionality of adapter and solver installations by executing them.

All v202211.0 test can be run successfully inside our NixOS VM and also in a \texttt{nix develop} environment, except \texttt{turek-hron-fsi2} and \texttt{flow-over-heated-plate-steady-state}.

We could not manage to build \texttt{swak4foam}, an add-on to OpenFOAM that is needed in the \texttt{turek-hron-fsi2} tutorial, from source.
The official preCICE VM uses a prebuilt version of swak4foam, however also patching the prebuilt binary did not work.

The second simulation we were not able to run is \texttt{flow-over-heated-plate-steady-state}.
This testcase needs code-aster to work, that we managed to package as already mentioned.
However, when running the simulation after executing the manual steps as written in the documentation, there occurs a segmentation fault, crashing code-aster when the python code tries to call a Fortran subroutine.
