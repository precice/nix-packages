...
There are a wide range of phases that always get executed in the same order and package maintainers can then decide weather the default phase is sufficient or needs to be overridden in order to produce a working build.
The most common phases to override are the \texttt{configurePhase}, which is mostly used for CMake or Autotools configuration, the \texttt{buildPhase}, \texttt{installPhase} and the \texttt{checkPhase} which can for example be used to run unit tests.
